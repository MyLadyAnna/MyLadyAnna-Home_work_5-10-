...
async def mes_start(message: types.Message):
    ...
    await message.answer(f'{name}, привет! Сегодня сыграем с тобой в конфеты! Для начала игры введи команду /new_game. '
                         f'Для настройки конфет введи команду /set и укажи количество конфет. Эту команду нужно ввести до начала игры!\n'
...
async def mes_duel(message: types.Message):
    ...
    global first
    global current
    if message.text.split()[1].isdigit() and len(message.text) == 15:
        ...
        await message.answer('Укажи корректный id! Он состоит из 9 ЦИФР.')
        ...
        if current == int(message.from_user.id):
            ...
            if new_game:
                if message.text.isdigit() and 0 < int(message.text) < 29:
                    ...
                    if total <= 0:
                        await message.answer(f'Ура! {name} ты победил(а)!')
                        await dp.bot.send_message(enemy_id(), 'К сожалению, ты проиграл(а)! Твой оппонент оказался умнее! :)')
                        new_game = False
                    else:
                        await message.answer(f'{name} взял(а) {message.text} конфет. '
                                             f'На столе осталось {total}')
...
async def bot_turn(message: types.Message):
    ...
    if 0 < total < 29:
        bot_take = total
        total -= bot_take
        await message.answer(f'Бот взял {bot_take} конфет и одержал победу')
        new_game = False
    else:
        remainder = total%29
